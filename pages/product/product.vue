	...
	<view class="container">
		...
		<view class="introduce-section">
			...
			<view class="price-box">
				...
				<text class="price">{{ specsTextShow ? activeSku.price / 100 : productDetail.show_price }}</text>
			</view>
			<view class="bot-row">
		...
		<!-- 操作菜单 -->
		<add-cart-buy @listenAddCart="addCart(productDetail)" @listenBuy="buy(productDetail)" :cartCount="cartCount" />
		...
		<u-popup v-model="specShow" mode="bottom" height="auto" border-radius="20" :closeable="true" close-icon-pos="top-right">
			<view class="attr-content">
				<view class="a-t">
					...
					<view class="right">
						<text class="price">￥{{ activeSku.price === undefined ? productDetail.show_price : activeSku.price / 100 }}</text>
...
export default {
	...
	methods: {
		...
		addCart(product) {
			...
			this.cart = Array.from(new Set(this.cart.map(item => item.id))).map(item => this.cart.find(items => items.id === item));
