<template>
	<view class="container">
		<!-- 商品轮播图 -->
		<view class="carousel">
			<u-swiper :list="productDetail.edges.goods_spu_imgs" name="img_path" mode="number"
				indicator-pos="bottomRight" height="722"></u-swiper>
			<!-- 返回按钮 -->
		...
		<view class="introduce-section">
			...
			<view class="price-box">
				...
				<text class="price">{{ specsTextShow?activeSku.price:productDetail.show_price}}</text>
			</view>
			<view class="bot-row">
				<text>销量：108</text>
				...
				<view class="con">
					<block v-if="specsTextShow">
						<block v-for="key in Object.keys(specsActive)" :key="key">
							<text class="selected-text">{{specsActive[key]['name']}}&nbsp;&nbsp;</text>
						</block>
					</block>
					<block v-else>
						请选择规格
		...
		<!-- 规格弹出层 -->
		<u-popup v-model="specShow" mode="bottom" height="auto" border-radius="20" :closeable="true"
			...
			<view class="attr-content">
				<view class="a-t">
					...
					<view class="right">
						<text
							class="price">￥{{ activeSku.price === undefined ? productDetail.show_price :activeSku.price }}</text>
		...
		<!-- 					<block v-for="(item,index) in activeSku.edges.goods_specs_option" :key="index">
				...
				<view class="attr-list" v-for="item in specs" :key="item.id">
					...
					<view class="item-list">
						<text v-for="childItem in item.list" :key="childItem.id" class="tit" :class="{
		...
		<!-- 服务弹出层 -->
		<u-popup v-model="serviceShow" mode="bottom" height="auto" border-radius="20" :closeable="true"
			close-icon-pos="top-right">
			<view class="attr-content">
				<view class="ser-list">
					<view class="item">
			...
			isAble(specs_id, option) {
				...
				let flag = this.skus.some(item => {
					...
					for (let k in copySelectSpec) {
						if (copySelectSpec[k] == "") {
							i++
						} else if (item.specs.findIndex((s_spec) => s_spec.id == copySelectSpec[k].id) > -1) {
							i++
						}
					}
					return i == specLen
